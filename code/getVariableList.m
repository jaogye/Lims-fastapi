% This function select varlist of the plots based on the selected context
% from the user 
...
if not( ismissing( VarId )) && VarId > 0 
   if idProduct ==  0 || idQuality ==  0  
      ...
      sWhere = " s.id=m.sample_id  and m.variable_id = %d  and s.loadingdate>='%s' and s.loadingdate<= '%s'" ;       
               
      sWhere = sprintf(sWhere,  VarId, sdateini, sdatefin) ; 
      if idProduct > 0
      ...
      if not( strcmp(bruto, '') ) 
         ...
         sWhere = sWhere + sprintf( " and s.product_id=p.id and replace( replace(p.bruto, '<sub>', ''), '</sub>', '')='%s' ", bruto) ; 
      end      
      sql = "SELECT s.id FROM " + sFrom + " WHERE " + sWhere ; 
   else    
      sql = "SELECT s.id FROM sample s, measurement m WHERE" + ...
         " s.product_id = %d and s.quality_id = %d and s.id=m.sample_id" + ...
         " and m.variable_id = %d  and s.loadingdate>='%s' and s.loadingdate<= '%s'" ; 
